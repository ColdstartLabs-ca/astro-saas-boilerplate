...
4. **For each PRD** (process ONE at a time, then stop):
   ...
   ```
   git worktree add ../autopilotrank.com-nw-<prd-name> night-watch/<prd-name>
   ```
   ...
   ```
   cd /home/joao/projects/autopilotrank.com
   git checkout main
   ...
   m. **Clean up worktree**: `git worktree remove ../autopilotrank.com-nw-<prd-name>`
