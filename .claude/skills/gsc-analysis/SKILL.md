...
node ~/.claude/skills/gsc-analysis/scripts/gsc-fetch.cjs --site=autopilotrank.com
```
...
# Custom date range
node ~/.claude/skills/gsc-analysis/scripts/gsc-fetch.cjs --site=autopilotrank.com --days=90

# Save to file
node ~/.claude/skills/gsc-analysis/scripts/gsc-fetch.cjs --site=autopilotrank.com --output=/tmp/gsc-apr.json
```

Logs go to stderr, data goes to stdout. Pipe: `node gsc-fetch.cjs --site=autopilotrank.com 2>/dev/null | jq .summary`
...
```bash
node ~/.claude/skills/gsc-analysis/scripts/gsc-fetch.cjs --site=autopilotrank.com --days=28 --output=/tmp/gsc-apr.json 2>&1
```

### Step 2: Read JSON and Analyze

After running, read `/tmp/gsc-apr.json` and analyze:
...
- Verify site is verified in GSC as `sc-domain:autopilotrank.com`
