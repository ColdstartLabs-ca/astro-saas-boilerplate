...
#### 3.4 `dashboard-billing.e2e.spec.ts` — needs investigation

**Root causes:**

1. **Stale selectors** — Tests for subscription states expect specific UI elements (`data-testid="modal"`, `div[role="dialog"]`) that don't match current billing page components.
2. **Error handling tests** (lines 540-574) — Expect error states/retry buttons that may not exist in current UI.
