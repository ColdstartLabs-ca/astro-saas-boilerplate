...
export function useCategories(enabled = true): IUseCategoriesReturn {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await adminFetch<{ categories: IBlogCategory[] }>(
...
export interface IUseUploadMediaReturn {
  uploadMedia: (file: File, altText?: string, tags?: string[]) => Promise<IUploadMediaResult | null>;
  isUploading: boolean;
  progress: number;
  error: string | null;
}
 ...
 */
export function useUploadMedia(onSuccess?: (media: IUploadMediaResult) => void): IUseUploadMediaReturn {
