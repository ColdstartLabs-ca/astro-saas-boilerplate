...
export function ArticleDetailModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fadeIn" data-testid="article-detail-modal">
      <div className="bg-surface border border-border rounded-xl w-full max-w-md shadow-2xl p-6 relative">
        ...
        <div className="mb-5">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className={`ml-auto text-xs px-2 py-0.5 rounded-full border ${statusConfig.bgClass} ${statusConfig.textClass} ${statusConfig.borderClass}`}>
              {statusConfig.label}
            </span>
          </div>
          ...
          {article.title && (
            <p className="text-xs text-muted mt-1">{article.primaryKeyword}</p>
          )}
        </div>
        ...
        {canPublish && (
          <div className="mb-4">
            ...
            {publishConfirm && !isPublishing && (
              <p className="text-xs text-muted text-center mt-2">Click again to confirm immediate publishing</p>
            )}
          </div>
        )}
