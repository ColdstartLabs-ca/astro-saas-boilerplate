...
import { PlayCircle, CheckCircle2, ArrowRight, Loader2, FileText, BarChart2, Calendar as CalendarIcon, Search, Check, LayoutGrid, Settings, Bell, User, Zap, Rocket, ChevronDown, Plus, Filter } from 'lucide-react';
  ...
  <div className="h-full flex flex-col animate-fadeIn">
    ...
    <div className="flex justify-between items-center mb-6 px-1">
      <div>
        <h3 className="text-white font-semibold flex items-center gap-2">
          ...
          <span className="px-2 py-0.5 rounded-full bg-brand-500/20 text-brand-400 text-[10px] font-mono border border-brand-500/30">RUNNING</span>
        </h3>
        <p className="text-slate-500 text-xs mt-1">Processing 3 campaigns • 12 articles remaining</p>
      </div>
      ...
      <div className="flex gap-4 h-full">
        ...
        <div className="flex-1 flex flex-col">
           ...
           <div className="flex-1 bg-slate-950/50 rounded-lg border border-slate-800/50 p-3 font-mono text-xs text-slate-400 leading-relaxed overflow-hidden relative">
             ...
             <span className={`inline-block w-1.5 h-3 bg-brand-500 ml-0.5 align-middle transition-opacity duration-100 ${cursorVisible ? 'opacity-100' : 'opacity-0'}`}></span>
        ...
        <div className="w-32 hidden sm:flex flex-col gap-2 border-l border-slate-800 pl-4">
           <div className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1">Workflow</div>
           {[
             ...
             <div key={i} className="flex items-center justify-between text-[10px]">
               <span className={step.status === 'active' ? 'text-brand-400 font-medium' : step.status === 'done' ? 'text-slate-400' : 'text-slate-600'}>
                 {step.label}
               </span>
               {step.status === 'done' && <Check className="w-3 h-3 text-green-500" />}
               {step.status === 'active' && <Loader2 className="w-3 h-3 text-brand-500 animate-spin" />}
             </div>
           ))}
        </div>
      </div>
    ...
    <div className="space-y-2">
      {[
        ...
        { title: "Email Marketing Guide", status: "Scheduled", badge: "bg-purple-500/10 text-purple-400" },
      ].map((item, i) => (
        <div key={i} className="flex items-center justify-between p-2.5 rounded-lg border border-slate-800 bg-slate-900/40 text-xs">
  ...
  <div className="h-full flex flex-col animate-fadeIn">
    <div className="flex justify-between items-center mb-5 px-1">
      ...
      <div className="flex gap-2">
         <button className="p-1.5 rounded hover:bg-slate-800 text-slate-400"><Filter className="w-4 h-4"/></button>
         <button className="p-1.5 rounded hover:bg-slate-800 text-slate-400"><LayoutGrid className="w-4 h-4"/></button>
      </div>
    </div>
    ...
    <div className="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden flex-1 flex flex-col">
      ...
      <div className="flex-1 overflow-hidden">
         {[
           ...
           <div key={i} className="grid grid-cols-12 gap-2 p-3 items-center border-b border-slate-800/50 hover:bg-slate-800/30 group transition-colors text-xs">
              ...
              <div className="col-span-2 text-right">
                 <span className={`px-1.5 py-0.5 rounded ${row.kd < 30 ? 'bg-green-500/10 text-green-400' : 'bg-yellow-500/10 text-yellow-400'}`}>
                   {row.kd}
                 </span>
              </div>
              <div className="col-span-3 flex justify-end items-center gap-2">
                 ...
                 <div className="h-4 w-12 flex items-end gap-[1px]">
                    {row.trend.map((h, j) => (
                      <div key={j} style={{height: `${h}%`}} className={`w-1.5 rounded-t-[1px] ${row.kd < 30 ? 'bg-brand-500/40' : 'bg-slate-700'}`}></div>
                    ))}
                 </div>
              </div>
           </div>
    ...
    <div className="flex gap-4 flex-1 min-h-0">
       ...
       <div className="flex-1 bg-slate-900 rounded-xl border border-slate-800 p-4 relative overflow-hidden">
          ...
          <div className="text-xs text-slate-400 leading-6">
            <p className="mb-2">
              Page speed is a <span className="bg-yellow-500/20 text-yellow-200 px-0.5 rounded border-b border-yellow-500/40">critical ranking factor</span> for Google.
              Slow sites lead to higher bounce rates and lower conversions.
            </p>
            <p>
              To improve performance, focus on <span className="bg-green-500/20 text-green-200 px-0.5 rounded border-b border-green-500/40">optimizing images</span> and leveraging browser caching.
            </p>
          </div>
       ...
       <div className="w-40 flex flex-col gap-3">
           <div className="bg-slate-900 rounded-xl border border-slate-800 p-4 flex flex-col items-center justify-center relative overflow-hidden group min-h-[140px]">
               ...
               <div className="relative w-28 h-28 flex items-center justify-center mb-1">
                   ...
                   {/* Main Progress SVG */}
                   <svg className="w-20 h-20 -rotate-90 relative z-10 drop-shadow-[0_0_10px_rgba(34,197,94,0.3)]" viewBox="0 0 100 100">
                       <defs>
               ...
               <div className="flex flex-col items-center relative z-10">
                    <div className="text-[10px] uppercase tracking-widest font-bold text-slate-500 mb-1">SEO Health</div>
               </div>
           </div>
  ...
  <div className="h-full flex flex-col animate-fadeIn">
    <div className="flex justify-between items-center mb-5 px-1">
      ...
      <div className="flex gap-2 text-slate-400">
        <button className="p-1.5 rounded hover:bg-slate-800"><ChevronDown className="rotate-90 w-4 h-4" /></button>
        <button className="p-1.5 rounded hover:bg-slate-800"><ChevronDown className="-rotate-90 w-4 h-4" /></button>
      </div>
    </div>
    ...
    <div className="flex-1 bg-slate-900 rounded-xl border border-slate-800 p-3">
       <div className="grid grid-cols-7 gap-1 h-full auto-rows-fr">
          {['S','M','T','W','T','F','S'].map(d => (
            <div key={d} className="h-6 text-[10px] font-bold text-slate-600 text-center flex items-center justify-center">{d}</div>
          ))}
          ...
          {Array.from({length: 30}).map((_, i) => {
             ...
             return (
               <div key={i} className={`min-h-[40px] rounded border ${isToday ? 'border-brand-500/30 bg-brand-500/5' : 'border-slate-800/50 bg-slate-950/30'} relative group hover:border-slate-600 transition-colors p-1 flex flex-col`}>
                  <span className={`text-[9px] ${isToday ? 'text-brand-400 font-bold' : 'text-slate-500'} mb-1`}>{day}</span>
    ...
    <div className="mt-3 flex gap-4 text-[10px] text-slate-500 px-1">
       <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded bg-brand-500"></div> Scheduled</div>
       <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded bg-slate-600 border border-slate-500 border-dashed"></div> Draft</div>
    </div>
  </div>
);

export function HeroSection({ className = '' }: { className?: string }): JSX.Element {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
        ...
        <div className={`transition-all duration-700 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
          <div className="inline-flex items-center space-x-2 bg-slate-900/50 border border-slate-700/50 rounded-full px-4 py-1.5 mb-8 backdrop-blur-md shadow-lg shadow-brand-500/10 hover:border-brand-500/50 transition-colors cursor-default group">
            ...
            <span className="text-sm font-medium text-slate-300 group-hover:text-white transition-colors">Trusted by 500+ businesses</span>
            <ArrowRight className="w-3 h-3 text-slate-500 group-hover:text-brand-400 transition-colors ml-1" />
          </div>
        </div>

        {/* Headline */}
        <h1 className={`text-4xl md:text-6xl lg:text-7xl font-bold tracking-tight text-white mb-6 leading-[1.15] transition-all duration-700 delay-100 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
          Scale Your Organic Traffic <br className="hidden md:block" />
          <span className="gradient-text">on Autopilot</span>
        </h1>

        {/* Subheadline */}
        <p className={`max-w-2xl mx-auto text-lg md:text-xl text-slate-400 mb-10 leading-relaxed transition-all duration-700 delay-200 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
          The only AI SEO platform that truly does it all – automating content creation, optimization, and publishing with human-level quality. No bugs. No generic content.
        </p>

        {/* CTAs */}
        <div className={`flex flex-col sm:flex-row items-center justify-center space-y-4 sm:space-y-0 sm:space-x-4 mb-16 transition-all duration-700 delay-300 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
          <motion.button
        ...
        {/* Trust Badges */}
        <div className={`flex flex-wrap justify-center gap-x-8 gap-y-4 text-sm text-slate-500 mb-20 transition-all duration-700 delay-400 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
          <div className="flex items-center hover:text-slate-300 transition-colors"><CheckCircle2 className="h-4 w-4 mr-2 text-brand-500" /> No credit card required</div>
          <div className="flex items-center hover:text-slate-300 transition-colors"><CheckCircle2 className="h-4 w-4 mr-2 text-brand-500" /> 14-day free trial</div>
          <div className="flex items-center hover:text-slate-300 transition-colors"><CheckCircle2 className="h-4 w-4 mr-2 text-brand-500" /> Cancel anytime</div>
        </div>

        {/* 3D Dashboard Container */}
        <div className={`relative mx-auto max-w-5xl transition-all duration-1000 delay-500 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}>
          <div className="relative rounded-2xl bg-slate-950 border border-slate-800 shadow-2xl shadow-black/80 aspect-[16/10] sm:aspect-[16/9] flex flex-col overflow-hidden group hover:border-slate-700 transition-colors">
             ...
             <div className="h-10 border-b border-slate-800 bg-slate-900/50 flex items-center justify-between px-4 select-none">
                 ...
                 <div className="flex items-center gap-2 bg-slate-950 border border-slate-800 rounded-md px-3 py-1">
                     ...
                     <span className="text-[10px] text-slate-400 font-medium">app.autopilotrank.com</span>
                 </div>
                 <div className="flex items-center gap-3">
             ...
             <div className="flex-1 flex overflow-hidden">
                 ...
                 <div className="w-16 sm:w-48 bg-slate-900/30 border-r border-slate-800 flex flex-col py-4 px-2 sm:px-3 gap-1">
                    {[
                      ...
                      >
                         <span className={activeSlide === item.id ? 'text-brand-400' : 'group-hover:text-slate-300'}>{item.icon}</span>
                         <span className="hidden sm:block text-xs font-medium">{item.label}</span>
                         {activeSlide === item.id && <div className="ml-auto w-1 h-1 rounded-full bg-brand-500 hidden sm:block"></div>}
                      </button>
                    ))}
                 ...
                 <div className="flex-1 bg-slate-950 relative overflow-hidden flex flex-col">
                    ...
                    <div className="flex-1 p-6 overflow-hidden relative">
                       ...
                       {activeSlide > 3 && <div className="flex items-center justify-center h-full text-slate-600 text-sm">Analytics Module Loading...</div>}
                    </div>
