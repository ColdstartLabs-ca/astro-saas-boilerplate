      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative max-w-6xl mx-auto mb-24">
          ...
          <div className="hidden md:block absolute top-[2.5rem] left-0 w-full h-0.5 bg-slate-800 z-0">
             <div className="absolute top-0 left-0 h-full w-1/3 bg-gradient-to-r from-transparent via-brand-500 to-transparent animate-scroll opacity-50" style={{ animationDuration: '3s' }}></div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-8 md:gap-4 relative z-10">
            {steps.map((step, idx) => (
              <div key={idx} className="flex flex-col items-center group cursor-default">
                ...
                <span className="text-[10px] font-mono uppercase tracking-wider bg-slate-800 px-2 py-0.5 rounded text-slate-400 border border-slate-700">Auto</span>
              </div>
            ))}
          </div>
        </div>
